...
async function processPendingActions(user) {
  ...
  // 2) Se c'è una company da creare (signup nuovo), creala e collega l'utente.
  // Importante: il controllo va fatto su company_members, non su companies generiche,
  // altrimenti un nuovo utente può restare senza azienda o finire in un flusso ambiguo.
  const metaCompany = user.user_metadata?.pending_company_name ? JSON.stringify({ name: user.user_metadata.pending_company_name, country: user.user_metadata.pending_company_country || "OTHER" }) : null;
  const pendingCompany = localStorage.getItem("pendingCompany") || metaCompany;
  if (pendingCompany) {
    try {
      ...
      if (!memberships || !memberships.length) {
        const { data: created, error: companyError } = await supabase
          .from("companies")
          .insert({ name: c.name, country: c.country, created_by: user.id })
          .select("id")
          .single();
        ...
        const { error: memberError } = await supabase
          .from("company_members")
          .insert({ company_id: created.id, user_id: user.id, role: "admin" });
        if (memberError) console.warn("Company creata, ma collegamento membro non riuscito:", memberError.message);
      }
      localStorage.removeItem("pendingCompany");
    } catch (e) { console.warn("Errore creazione azienda post-signup:", e); }
  }
}
...
export async function createShareLink(projectId) {
  const user = await currentUser();
  const { data: owned, error: ownedError } = await supabase.from("projects").select("id").eq("id", projectId).eq("owner", user.id).maybeSingle();
  if (ownedError) throw ownedError;
  if (!owned) throw new Error("Progetto non trovato o non autorizzato");
